...
run_rust_type_narrowing_checks() {
  ...
  local output
  # Inline the helper script
  output="$(python3 - "$PROJECT_DIR" --ast-json "${guard_json:-/dev/null}" <<'PYHELPER' 2>&1
#!/usr/bin/env python3
...
def main() -> int:
    ...
    if len(sys.argv) >= 4 and sys.argv[2] == "--ast-json":
        ...
        if json_path_arg and json_path_arg != "/dev/null":
             json_path = Path(json_path_arg).resolve()

    issues: List[tuple[Path, int, int, str]] = []
    if json_path:
        issues = analyze_with_ast_json(root, json_path)
    # If no AST or AST failed to find findings (optional logic, but keeping existing behavior: fallback if ast not used?)
    # Actually, the original logic was: "if guard_json, use it. if not, use regex."
    # Here we can do both or fallback.
    # Original logic: if json_path provided, analyze_with_ast_json.
    # if not issues (and json_path was None?), analyze_with_regex.
    # Let's keep it simple: if JSON provided, use it. if not, regex.
    # But wait, if json is provided but empty?
    
    if not issues and not json_path:
